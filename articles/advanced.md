...
`covMethod = "r"` computes a numerical Hessian after optimisation and
reports standard errors in `print(fit)`. As of this version the default
is `covMethod = "r,s"` (see below), which reports the sandwich
correction of that same Hessian; pass `covMethod = "r"` explicitly to
get the uncorrected SEs. SEs are reported for the structural,
residual-error **and** omega (IIV) parameters; omega is reported on its
natural scale, with the Hessian rotated off the Cholesky factor the
optimiser works in. A larger `cov_n_sim` reduces MC noise in the
Hessian:
...
from `N(yt, Vt)`, so it assumes each subject’s observation vector is
multivariate normal. It is not — `y_i = f(theta, b_i) + eps_i` is
nonlinear in the random effect, so the marginal is a mixture. This does
**not** cost the point estimates: the score has expectation zero at the
true parameters under any weight, so the fit stays consistent. It costs
the reported uncertainty, in two ways that differ in kind:
`Cov(V_ij, V_kl)` is mis-sized by the excess kurtosis, and
`Cov(ybar, vech V)` is assumed zero where a real correlation of 0.3–0.6
sits. The sample mean and the sample covariance are exactly independent
...
Point estimates and the objective are identical to the `"r"` fit; only
the SEs move. Under correct specification `J = 2H` and `"r,s"` returns
exactly what `"r"` would have, so the two agreeing is the expected
outcome for a well-specified model, not a sign that nothing happened.

It applies to every residual family whose conditional law is independent
...
`logitNorm`, `probitNorm`). Anything it cannot build — an
[`ar()`](https://rdrr.io/r/stats/ar.html) residual, a
[`t()`](https://rdrr.io/r/base/t.html) with `nu <= 4`, an `ordinal()` or
