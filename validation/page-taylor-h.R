...
## h controls the WT range the per-node data SPANS. With h = sdlog/2 the three
## Taylor nodes sit at WT ~ 16-25 against a distribution covering roughly 8-49,
## so vpwt and qwt -- coefficients on parameters with NO random effect, which are
## identified only by how predictions change across weight -- have almost no
## range to be identified from.
RUNS <- list()
for (f in c(0.5, 1.0, 2.0))
  RUNS[[sprintf("taylor_h%.1f", f)]] <-
    local({ ff <- f
      a <- datagen(list(Ayuthaya = list(model = mod_iss, times = T_ISS,
                                        ev = EV_ISS, n = N_ISS,
                                        covariate = list(WT = WT_ISS))),
                   control = DG, quad_method = "taylor", n_nodes = 3L,
                   h = WT_ISS$sdlog * ff, order = 2L)
      b <- datagen(list(Alsultan = list(model = mod_als, times = T_ALS,
                                        ev = EV_ALS, n = N_ALS,
                                        covariate = list(WT = WT_ALS))),
                   control = DG, quad_method = "taylor", n_nodes = 3L,
                   h = WT_ALS$sdlog * ff, order = 2L)
      c(a, b) })
for (nm in names(RUNS))
  kv(sprintf("wtrange_%s", nm),
     sprintf("WT nodes %s", paste(sprintf("%.1f",
       sort(vapply(RUNS[[nm]][1:3], function(s) s[["cov"]]$WT, numeric(1)))),
       collapse = " ")))
