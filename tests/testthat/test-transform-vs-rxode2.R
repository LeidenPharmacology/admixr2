...
test_that("forward transforms match rxode2's exported ones", {
  ...
  for (lam in c(-0.5, 0, 0.3, 0.5, 1, 1.5, 2, 2.5)) {
    expect_equal(admixr2:::.admTBS(y, lam, 0L, 0, 1), rxode2::boxCox(y, lam),
                 tolerance = 1e-12, info = paste("boxCox lambda =", lam))
    expect_equal(admixr2:::.admTBS(y, lam, 1L, 0, 1), rxode2::yeoJohnson(y, lam),
  ...
  for (lam in c(-0.5, 0, 0.5, 1, 2, 2.5))
    expect_equal(admixr2:::.admTBS(yn, lam, 1L, 0, 1), rxode2::yeoJohnson(yn, lam),
  ...
  for (b in list(c(0, 20), c(-1, 1), c(2, 40))) {
    expect_equal(admixr2:::.admTBS(seq(b[1] + 0.01, b[2] - 0.01, length.out = 9), 1, 4L, b[1], b[2]),
                 rxode2::logit(seq(b[1] + 0.01, b[2] - 0.01, length.out = 9), b[1], b[2]),
                 tolerance = 1e-10, info = paste("logit", b[1], b[2]))
    expect_equal(admixr2:::.admTBS(seq(b[1] + 0.01, b[2] - 0.01, length.out = 9), 1, 6L, b[1], b[2]),
...
test_that("inverse transforms match rxode2's exported ones", {
  ...
  for (lam in c(0, 0.3, 0.5, 1, 1.5, 2, 2.5)) {
    ...
    expect_equal(admixr2:::.admTBSi(z, lam, 0L, 0, 1), rxode2::boxCoxInv(z, lam),
                 tolerance = 1e-10, info = paste("boxCoxInv lambda =", lam))
    expect_equal(admixr2:::.admTBSi(z, lam, 1L, 0, 1), rxode2::yeoJohnsonInv(z, lam),
                 tolerance = 1e-10, info = paste("yeoJohnsonInv lambda =", lam))
  }
  for (b in list(c(0, 20), c(-1, 1), c(2, 40))) {
    expect_equal(admixr2:::.admTBSi(z, 1, 4L, b[1], b[2]), rxode2::expit(z, b[1], b[2]),
                 tolerance = 1e-12, info = paste("expit", b[1], b[2]))
    expect_equal(admixr2:::.admTBSi(z, 1, 6L, b[1], b[2]), rxode2::probitInv(z, b[1], b[2]),
...
test_that("the inverse derivative matches a finite difference of rxode2's inverse", {
  ...
  for (lam in c(0.3, 0.5, 1, 1.5)) {
    expect_equal(admixr2:::.admTBSid(z, lam, 0L, 0, 1),
                 fd(function(q) rxode2::boxCoxInv(q, lam), z),
                 tolerance = 1e-5, info = paste("d boxCoxInv lambda =", lam))
    expect_equal(admixr2:::.admTBSid(z, lam, 1L, 0, 1),
                 fd(function(q) rxode2::yeoJohnsonInv(q, lam), z),
                 tolerance = 1e-5, info = paste("d yeoJohnsonInv lambda =", lam))
  }
  expect_equal(admixr2:::.admTBSid(z, 1, 4L, 0, 20), fd(function(q) rxode2::expit(q, 0, 20), z),
               tolerance = 1e-5)
  expect_equal(admixr2:::.admTBSid(z, 1, 6L, 0, 20), fd(function(q) rxode2::probitInv(q, 0, 20), z),
               tolerance = 1e-5)
})
...
test_that("transforms preserve dim() and handle non-finite input like rxode2", {
  ...
  for (yj in c(0L, 1L, 2L, 4L, 6L)) for (lam in c(0, 0.5, 1, 2)) {
    for (f in list(admixr2:::.admTBS, admixr2:::.admTBSi, admixr2:::.admTBSid)) {
      out <- f(m, lam, yj, 0, 40)
  ...
  for (yj in c(0L, 1L, 4L)) {
    expect_true(is.na(admixr2:::.admTBSi(c(1, NA), 0.5, yj, 0, 40)[2L]))
    expect_true(is.na(admixr2:::.admTBS(c(1, Inf), 0.5, yj, 0, 40)[2L]))
  }
})
...
test_that("_eps matches rxode2's, and is not confused with DBL_EPSILON", {
  ...
  expect_identical(admixr2:::.ADM_EPS, sqrt(.Machine$double.eps))
  ...
  expect_gt(admixr2:::.ADM_EPS / .Machine$double.eps, 1e7)
})
