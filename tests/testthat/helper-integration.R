...
#
# Eight fits, one per (estimator, covMethod) pair, on ONE well-specified study.
# Two properties are being set up, and both need the study to be well specified:
#
#  1. "r,s" must change the reported uncertainty and NOTHING else. It replaces
#     the filling of the covariance, computed after convergence on p_hat, so the
#     optimizer never sees it.
#  2. Under CORRECT specification J = 2H and "r,s" must return what "r" returns.
#     That reduction is the strongest end-to-end statement available: it runs the
#     whole G / Omega / H path, and a wrong G, a wrong Omega or a mismatched H
#     would all break it.
#
# THE STUDY IS GENERATED BY datagen(method = "gh"), NOT WRITTEN DOWN, and that is
# load-bearing twice over. An analytic V (say a flat 30% CV) is not what the
# model predicts, so (2) has nothing to hold; and it leaves `add.err` contributing
# 0.01 variance against ~1.7 from IIV, which is the trap test-integration-cov.R
# already documents -- H[sigma,sigma] near zero.
#
# THAT TRAP IS WORSE FOR "r,s" THAN FOR "r", which is why the sigma level matters
# here and does not elsewhere. "r" reports 2H^-1 and inverts an ill-conditioned H
# ONCE; the sandwich reports H^-1 J H^-1 and inverts it TWICE, so any gap between
# J and 2H in a near-null direction is amplified quadratically. Measured on the
# two fixtures at the same model and design:
#
#            analytic V, add.err = 0.1     datagen V, add.err = 1.0
#   diag(H)[add.err]        0.238                    548
#   cond(H)               351,600                    247
#   SE ratio, add.err        0.115                  1.0000
#   SE ratio, omega       0.59 / 1.55         1.0056 / 1.0083
#
# The 0.115 is not a correction, it is an unidentified direction being divided by
# twice. `add.err = 1` against an IIV-driven V of the same order is what makes the
# residual identified (RSE 3.3% rather than 186%) and the equality meaningful.
#
# `covMethod` is read off the FIT rather than off the control, because a requested
# sandwich that could not be built degrades to "r" silently by design -- so "was it
# applied" is a property of the result.
.int_sandwich_setup <- function() {
  ...
  # The helper model with a residual the design can actually identify.
  sand_fn <- function() {
    ini({ tcl <- log(5); tv <- log(20); add.err <- 1.0
          eta.cl ~ 0.09; eta.v ~ 0.04 })
  ...
  run <- function(est, control)
    suppressMessages(suppressWarnings(
      nlmixr2(sand_fn, admData(), est = est, control = control)))

  ctl <- function(est, cm) switch(est,
    adgh   = adghControl(studies = st, n_nodes = 7L, maxeval = 200L,
                         seed = 1L, grad = "analytical", covMethod = cm),
    admc   = admControl(studies = st, n_sim = 3000L, maxeval = 200L,
                        seed = 1L, grad = "sens", covMethod = cm,
                        cov_n_sim = 10000L),
    adfo   = adfoControl(studies = st, maxeval = 200L,
                         grad = "none", covMethod = cm),
    adirmc = adirmcControl(studies = st, n_sim = 3000L,
                           phases = c(1, 0.5), outer_iter = 20L, seed = 1L,
                           covMethod = cm))
  ...
  .int_sandwich_cache <<- list(ests = ests, fits = fits, studies = st, fn = sand_fn)
  .int_sandwich_cache
}
