...
test_that("an enrolled range keeps the margins it was applied to", {
  pop <- .pnd_pop()
  cd  <- admixr2:::.admCovApplyRange(pop, list(WT = c(55, 105)))
  ut  <- cd[[".adm_untrunc"]]
  # Recorded, and metadata: a collapse needs the pre-range margins to read a
  # latent loading off, but they must never be seen as a covariate.
  expect_false(".adm_untrunc" %in% admixr2:::.admCovSpecNames(cd))
  expect_named(ut, "WT")
  expect_false(is.null(ut$WT$meanlog))      # as it was
  expect_true(is.null(cd$WT$meanlog))       # as it now is
  expect_false(is.null(admixr2:::.admCovDistCanon(cd)[[".adm_untrunc"]]))
})

test_that("a truncated study collapses its integration instead of paying the grid", {
  skip_on_cran(); skip_if_not_installed("rxode2")
  # .admCovCollapse() declines a bent latent map, so a study with a `range` fell
  # back to the full product grid on EVERY evaluation. The span is still there
  # on the margins as they were; only the measure changed.
  a  <- .pnd_pinfo(.pnd_allom)
  cdT <- admixr2:::.admCovApplyRange(.pnd_pop(),
           list(WT = c(55, 105), CRCL = c(60, 130), ALB = c(31, 49)))
  expect_null(admixr2:::.admCovCollapse(a$ui, a$pinfo, cdT, 7L))
  co <- admixr2:::.admCovCollapseTrunc(a$ui, a$pinfo, cdT, 7L)
  expect_false(is.null(co))
  expect_true(isTRUE(co$recombined))
  expect_lt(nrow(co$X), 7L^3L)              # against the grid it replaces
  expect_equal(sum(co$W), 1, tolerance = 1e-12)
  # And it integrates what the grid integrates: the covariate-driven quantity,
  # against a finer reference on the same truncated margins.
  cn <- c("WT", "CRCL", "ALB")
  f <- function(X) {
    cl <- 3.2 * (X[, "WT"]/70)^.6 * (X[, "CRCL"]/95)^.4 * (X[, "ALB"]/40)^.3
    cbind(cl, log(cl), (200/21) * exp(-(cl/21) * 8))
  }
  grid_at <- function(J) {
    g <- admixr2:::.adghNodeGrid(J, 3L)
    R <- admixr2:::.admCovDistCanon(cdT)[["latentR"]] %||% diag(3L)
    z <- g$X %*% chol(R)
    X <- vapply(seq_along(cn), function(k)
      admixr2:::.admCovQuantile(cdT[[cn[k]]], admixr2:::.admCovU(z[, k])),
      numeric(nrow(z)))
    colnames(X) <- cn
    as.numeric(crossprod(g$W / sum(g$W), f(X)))
  }
  ref <- grid_at(21L)
  got <- as.numeric(crossprod(co$W / sum(co$W), f(co$X[, cn, drop = FALSE])))
  expect_lt(max(abs(got / ref - 1)), 1e-4)
  # at least as good as the grid it replaces, on far fewer rows
  expect_lte(max(abs(got / ref - 1)), max(abs(grid_at(7L) / ref - 1)))
})
