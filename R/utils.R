...
# rxode2's global compiled-model registry environment (`.rxModels`), or NULL if
# it cannot be reached (an older/newer rxode2 that renamed it). Accessed via
# get(..., asNamespace()) rather than the `:::` operator so R CMD check does not
# flag it; guarded so a rename degrades to "teardown does nothing" rather than an
# error.
.admRxRegistry <- function()
  tryCatch({
    .r <- get(".rxModels", envir = asNamespace("rxode2"))
    if (is.environment(.r)) .r else NULL
  }, error = function(e) NULL)

# Snapshot of the registry's keys. Call at the START of a driver, BEFORE admixr2
# loads any model, so the teardown can remove only the entries THIS fit added.
# Returns NULL when the registry is unreachable -> teardown then skips the clear.
.admRegistrySnapshot <- function() {
  .r <- .admRxRegistry()
  if (is.null(.r)) return(NULL)
  ls(.r, all.names = TRUE)
}

# Drop admixr2's in-memory model pins (`.adm_pin_env`). Shared by admClearCache().
.admClearPins <- function() {
  .nms <- ls(envir = .adm_pin_env, all.names = TRUE)
  if (length(.nms)) suppressWarnings(rm(list = .nms, envir = .adm_pin_env))
  invisible()
}

# Record the registry keys that appeared during a model load (between the `before`
# snapshot and now) as admixr2-created, in `.adm_model_keys`. Called on exit from
# .admLoadModel / .admLoadSensModel so that models admixr2 loads OUTSIDE an
# estimator fit (e.g. test-helper setup, or datagen's per-study loop) are still
# reclaimed by the next teardown -- while models the user registered themselves
# are never recorded and so never touched.
.admTrackRegistry <- function(before) {
  if (is.null(before)) return(invisible())
  .r <- .admRxRegistry()
  if (is.null(.r)) return(invisible())
  for (.k in setdiff(ls(.r, all.names = TRUE), before))
    assign(.k, TRUE, envir = .adm_model_keys)
  invisible()
}

# Per-fit teardown: reset the model state THIS fit accumulated, so a process that
# runs many fits does not climb without bound (the ubuntu-devel test-suite OOM).
#
# The blow-up trigger, isolated empirically (CI: pins-only vs registry-only clear
# before a heavy multi-output fit): it is the size of rxode2's GLOBAL model
# registry (`.rxModels`), NOT admixr2's own pins. admixr2 compiles several models
# per fit (the simulation model, and the foceiModel companions -- inner/outer/
# predOnly/predNoLhs -- read for sensitivities), each of which registers there.
# Once the registry holds a few hundred entries, the next fit's solve/output
# construction balloons the R heap by many GB (measured: registry at 286 ->
# multi-output fit heap +7 GB -> OOM; registry cleared first -> the SAME fit stays
# flat and completes). nlmixr2est bounds the registry with `rxUnloadAll()`, but
# its sweep of `.rxModels` fires only once >= getOption("rxode2.checkOrphans", 75)
# DLLs are loaded -- a threshold admixr2's workload sits under -- so it is never
# swept.
#
# We remove exactly the models ADMIXR2 registered, never the user's own:
#   * `before` is the registry-key snapshot taken at driver entry; the keys added
#     since then are what this fit registered (companions, sim/sens models).
#   * `.adm_model_keys` additionally records models admixr2 loaded OUTSIDE a fit
#     (test-helper setup, datagen's per-study loop) via .admTrackRegistry.
# The union of the two is dropped from the registry; any model the user or the
# nlmixr2 framework registered themselves is in neither set and is left untouched.
# The compiled DLLs stay loaded, so the next fit of the same model re-registers
# cheaply from the qs2 disk cache; a distinct model recompiles as it always would.
#
# We also drop admixr2's in-memory pins (large compiled-model R objects held per
# digest, never evicted) to release the R heap. But ONLY off Windows: on Windows
# the pins additionally anchor the foceiModel companions so their finalizers can
# never unload a DLL mid-allocation (STATUS_HEAP_CORRUPTION); there we leave them
# for admClearCache() to release. Off Windows the finalizers are safe, and
# dropping the pins lets the companion DLLs be reclaimed too. Registered as an
# on.exit in each estimator driver, so it runs after the fit object + covariance +
# output construction are complete.
.admFitTeardown <- function(before = NULL) {
  if (!isTRUE(getOption("admixr2.fit_teardown", TRUE))) return(invisible())
  .reg <- .admRxRegistry()
  if (is.null(.reg)) {
    # rxode2 renamed/removed .rxModels: warn once so the memory fix silently
    # disappearing is at least visible, then behave as a no-op.
    if (!exists("rxmodels_unreachable", envir = .adm_warn_env, inherits = FALSE)) {
      assign("rxmodels_unreachable", TRUE, envir = .adm_warn_env)
      warning("admixr2: could not reach rxode2's model registry to reclaim it ",
              "after fitting; long sessions may accumulate memory. ",
              "options(admixr2.fit_teardown = FALSE) silences this.", call. = FALSE)
    }
  } else {
    .fit_added <- if (is.null(before)) character(0)
                  else setdiff(ls(.reg, all.names = TRUE), before)
    .tracked   <- ls(.adm_model_keys, all.names = TRUE)
    .drop      <- intersect(union(.fit_added, .tracked), ls(.reg, all.names = TRUE))
    if (length(.drop))    suppressWarnings(rm(list = .drop,    envir = .reg))
    if (length(.tracked)) suppressWarnings(rm(list = .tracked, envir = .adm_model_keys))
  }
  if (!identical(.Platform$OS.type, "windows")) .admClearPins()
  gc(FALSE)
  invisible()
}
