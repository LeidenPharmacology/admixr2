...
#
# Names, fix flags, error types, and the fixed thetas' VALUES: a fix()ed theta
# never reaches the optimizer, so it travels to the solve as DATA on the
# cached object. Without it, `tka <- fix(0.5)` and `tka <- fix(0.9)` key
# alike, and a parallel worker with no `ui` to re-derive from solves every
# restart at the other fit's value, across sessions (rxTempDir() persists).
#
...
# outlive a change to what this package emits. Replaces a hand-maintained
# schema tag that had to be edited whenever the emitted model changed -- and
# was forgotten once, serving a stale entry after exactly such a change.
#
# The version ALONE is not enough: `Version:` moves only at release, so a
# whole dev cycle shares one key against a cache that persists across
# sessions. Editing .g2, the direction set, or the f2 naming would then hit
# the previously compiled model, and .adfoGrad would contract the OLD
# model's second-order columns against the new direction map -- a finite,
# plausible, silently wrong gradient. So the key also digests the BODIES of
# the functions that decide what gets emitted; any edit to one changes the
# key by construction. Costs one deparse + digest per fit.
.admPkgKey <- function() {
  .ver <- tryCatch(as.character(utils::packageVersion("admixr2")),
                   error = function(e) "dev")
  # deparse(), not the closure: a function's environment (the package
  # namespace) digests differently between load_all() and an installed build.
  #
  # Driven off a NAME LIST since digesting just the two entry points misses
  # helpers that shape the payload (e.g. .admJumpCovers()). Constants are
  # digested BY VALUE, per-name tryCatch so one unresolvable name doesn't
  # disable invalidation entirely.
  #
  # A LOCAL, not package-level constant: .admDaemonRestart() patches a dev
  # body into the stale installed namespace with assignInNamespace(), which
  # can REPLACE a binding but not ADD one -- a top-level constant would go
  # missing in the worker and silently compute a different key.
  #
  # RULE: a function belongs here if changing its BODY changes the cached
  # payload independent of the rest of the key -- a missed name causes a
  # cache HIT on stale code via extraction or growth. .admUnpairedThetas/
  # .admMuRefPairs/.admIniKey are covered indirectly via their keyed results.
  .emitters <- c(
...
#
# rxode2 names a model's .c/.so from `.rxPre(model, modName)`, which for an
# ANONYMOUS model is the parsed text ALONE -- it can't see inputs like the
# event-sensitivity code injected afterward. Two builds of one text differing
# only in those inputs land on a single .so; the later build wins for both,
# and since entry points resolve BY NAME an object bound to the earlier
# build silently starts executing the replacement (nlmixr2/rxode2#1171).
#
# admixr2 hits this hardest: .admSensFromInner() recompiles nlmixr2est's OWN
# inner model text with eventSens = "jump" where nlmixr2est built it with a
# different one -- same parsed md5, same directory, different emitted C, and
# rxTempDir() persists across sessions, so a collision survives restarts.
...
#
# NOT rxTempDir(): something in a session invalidates artifacts inside it --
# upstream measured a generated model's .so replaced mid-run by a build
# emitting different event-sensitivity code, even with unique names. For
# admixr2 it's also a PERSISTENT user cache.
#
# R's session temp directory is neither: nothing else clears it, and it goes
# away when R exits. Cost: artifacts aren't shared across sessions, so each
# rebuilds once -- worth it while nlmixr2/rxode2#1171 is open, vs. a
# silently wrong gradient. ("Persists", used below: rxTempDir() is genuinely
# persistent only once the user has run rxCreateCache(); otherwise the
# cross-session scenarios below only apply on a machine with an rxode2 cache.)
#
# The .rds caches stay in rxTempDir(), so a cross-session hit may reference a
# DLL this session no longer has. .admRxLoadAll() checks file.exists(rxDll())
# and reports the entry stale -- required because rxLoad() on a vanished DLL
# doesn't reliably error; it quietly binds to whatever shares the entry-point
# name and solves to garbage. A DELIBERATE DIVERGENCE from upstream; don't
# "restore parity" by moving either half.
#
# nlmixr2est keeps one invariant admixr2 doesn't: an artifact's lifetime
# equals its cache's. admixr2 crosses them for the SENSITIVITY model only
# (session-local artifact, persistent adm-sens-*.rds); the simulation model
# stays on upstream's disk pairing.
#
# Three options considered:
#   A  session-only sens cache, upstream's. A mirai worker can't read a
#      session env (only ui_lstExpr, not `ui`), so parallel grad = "sens"
#      would break.
#   B  add a session token to the cache file's NAME, so a new session misses
#      and rebuilds rather than seeing a foreign entry.
#   C  keep the cross-session cache and guard it at runtime.  <-- CHOSEN,
#      for the cold-start win, at the cost of .admRxLoadAll()'s checks below.
#
...
# library, and it is the CONTAINER that gets cached -- so every rxode2-classed
# element must be re-loaded, not just the one the caller reads first. admixr2
# loaded exactly one element by name, correct today and silently wrong once a
# second is added.
#
# TRUE if everything loaded; FALSE if any load failed, which callers treat as
# a stale cache entry (delete and rebuild).
# Canonical spelling of a path, for comparing two of them.
#
# normalizePath() resolves Windows 8.3 short components ("RT6EC4~1") against
# the long form tempdir() reports; it warns and returns its input for a path
# that doesn't exist, so suppress and compare that. Case-folded on Windows
# ONLY, since its file system is case-insensitive; folding elsewhere would
# make two GENUINELY different directories compare equal below.
...
#
# More than one build directory counts as "this session": admixr2's own
# .admModDir() and nlmixr2est 7.x's <tempdir>/nlmixr2estSens. Testing
# .admModDir() alone rejected the second, so a cached .admSensFromInner()
# result was reported stale on EVERY call in the session that built it, and
# the model recompiled every fit. Hence a prefix test against the tempdir,
# not a fixed depth.
#
# BOTH SPELLINGS OF tempdir() are needed: normalizePath() resolves symlinks
# only for a path that EXISTS. On macOS tempdir() is a symlink, so tempdir()
# itself resolves while a not-yet-built artifact path under it doesn't,
# failing a prefix test against its own root.
...
.admRxLoadAll <- function(x) {
  .one <- function(e) {
    ...
    # model whose shared object has gone -- it silently re-runs the deferred
    # compile, so the caller can't tell a hit from a rebuild. Measured cost:
    # ~2.9 s, and with N daemons reading one .rds whose artifact is missing,
    # all N recompile concurrently into the SAME output path. (Role-tagged
    # modNames make the entry-point prefix unique per role, so the #1171
    # name-collision branch itself can't fire for our own models -- this
    # check exists for the two reasons below instead.)
    .dll <- tryCatch(rxode2::rxDll(e), error = function(err) NA_character_)
    if (is.na(.dll) || !nzchar(.dll) || !file.exists(.dll)) return(FALSE)
    # file.exists() alone is not the invariant: R removes its temp directory
    # only on a CLEAN exit, so a killed session leaves its build directory
    # behind indefinitely, and a concurrently LIVE second session's directory
    # is equally readable and vanishes under us when that session exits. The
    # .rds caches persist while these artifacts are session-local, so the
    # isolation .admModDir() implies has to be enforced, not assumed.
    #
    # Discriminated on the PATH, not the basename: a basename test for
    # "admSens" misses .admRxode2()'s anonymous-naming fallback (still built
    # inside .admModDir()) and nlmixr2est's own inner model
    # (tempdir()/nlmixr2estSens on 7.x). Tested against the SESSION TEMPDIR
    # via .admUnderTemp(), the one thing common to both *Sens directories.
    #
    # NORMALISE BEFORE MATCHING: rxDll() hands back a path in Windows 8.3
    # short form, so a literal grepl("admixr2Sens", .) never matches and the
    # guard silently does nothing. Verified by a two-process test: without
    # this the second session accepts the first session's model.
...
# alone isn't enough: `tka <- fix(0.5)` and `tka <- fix(0.9)` give the same
# digest(ui$lstExpr), and since a fixed theta never reaches the optimizer the
# solve uses whatever value rxode2 BAKED INTO $simulationModel -- so the
# second fit silently read the first's cache entry and solved at its fixed
# value, across sessions. Same collision .admIniKey() was added to close for
# the SENSITIVITY cache; only the simulation cache had been left on the old key.
#
# Split out as its own function because a parallel worker cannot recompute
# it: it has no `ui`. The parent stores this path on pinfo and
# .admWorkerLoadModels() reads it from there -- an earlier attempt enriched
# the key here and left the worker recomputing the old formula.
...
.admLoadModel <- function(ui) {
  ...
  #
  # STAYS rxTempDir(): upstream moved its generated models out of that
  # directory (see .admModDir()) by passing `wd =` to rxode2::rxode2(), never
  # by setwd(). Doing it here with setwd() breaks multi-endpoint models --
  # `rxode2(ui)$simulationModel` compiles companion models that resolve
  # against the working directory, and moving it made every multi-output fit
  # return an all-NA structural gradient. So the build-directory change
  # applies only where admixr2 emits the model text itself and can pass
  # `wd =` explicitly (.admRxode2()), not to this call.
...
#
# nlmixr2's output machinery deep-clones the ui with nlmixr2est's internal
# .cloneEnv(), recursing into every environment-valued member with no cycle
# detection. rxode2's compiled model objects hold a back-reference to the
# global .rxModels registry (registry -> model -> .rx -> .rxModels ->
# registry ...), so cloning one loops forever ("infinite recursion" / "node
# stack overflow"). A normal nlmixr2 fit never populates ui$meta with these;
# admixr2 does, because it simulates via $simulationModel. Safe to drop:
# rxode2 regenerates these lazily if a downstream method needs them.
...
#
# linCmt()'s first derivative resolves through rxode2's linCmtB rules, but
# there is no second one, so adfo's struct-theta pass would be stuck
# finite-differencing (nlmixr2est refuses linCmt outright). The ODE form has
# ordinary state sensitivities instead, so .rxSens() expands it to any order.
#
# `rxode2::linToOde()`'s output can't be used as-is: it names the prediction
# `rxLinCmt`, and an `rx`-prefixed lhs is RESERVED, so ui$loadPruneSens dies
# with "syntax errors". Renaming that variable is the whole fix. Reproduces
# the analytic linCmt prediction to 1.8e-08 relative on a 1-cmt oral model.
#
...
# "jump"); otherwise the column is silently ZERO. FEATURE-DETECT rather than
# version-compare: the compiled model carries eventSensInfo$derivs, and an
# unsupported modifier has zero rows.
#
# Tested per DIRECTION, not "the model has a lag()": nlmixr2est's inner
# model has no theta directions, so its derivs$lag is legitimately empty for
# `alag(depot) = exp(tlag)`.
#
# FALSE -> caller returns NULL for the whole sens model and the estimators
# fall back to a finite-difference gradient -- far better than a silently
# zero gradient component.
...
# d(pred)/d(eta) -- so it reuses its eta's column for free. Only a theta
# with no usable eta gets its own. Sigmas get none. Same scheme as
# nlmixr2est's fast-focei (.foceiAnalyticDirections).
#
# `order = 2L` additionally emits the CROSS second-order block
#   rx_f2_<eta_i>_<dir_b> = d2(pred)/(d eta_i d dir_b)
# which only adfo needs: its objective is built on V_pred = J Omega J' +
# resid(mu, diag(J Omega J')) with J = df/d(eta)|_0, so a structural theta's
# gradient needs dJ/d(theta) = d2f/(d eta d theta). admc/adgh read theirs off
# the first-order columns and stay at order 1. The block is deliberately
# ASYMMETRIC -- eta directions x ALL directions, not the full symmetric
# triangle nlmixr2est's builder needs for the Laplace Hessian -- since adfo
# differentiates only ONCE w.r.t. a theta and needs none of the
# rx_rvar*/rx_rsig* variance chains that dominate FOCEI's order-2 compile time.
#
# Two branches: an ODE model has rxode2::.rxSens() augment the system with
# variational (state-sensitivity) compartments, emitting
#   rx_f1_<dir> = d(pred)/d(dir) + sum_states d(pred)/d(state) * d(state)/d(dir)
# A linCmt model has no states to augment (.rxSens errors), so the state sum
# drops out and D(pred, dir) alone is emitted, resolved through rxode2's
# linCmtB derivative rules -- why the direction set works for linCmt at
# first order even though nlmixr2est's augmented outer model can't build it.
#
...
# without it such a sensitivity is silently ZERO. State initial conditions
# and their direction derivatives are emitted too (a parameter-dependent IC
# otherwise starts every sensitivity compartment at 0).
#
# Returns list(mod, dirs, sens_cols, theta_sens_cols) or NULL on any failure
# (caller falls back to nlmixr2est's inner model + FD). `pred_expr`: the
# model expression whose sensitivities to emit, as a symengine object.
# Defaults to `rx_pred_`, the prediction for every ordinary endpoint -- but
# NOT for a likelihood-form endpoint, where rxode2 puts the LOG-LIKELIHOOD
# there; FOCEI wants exactly that, admixr2 moment-matches and needs the
# MEAN, so such an endpoint passes its own derived expression. This works
# because .rxSens() builds the variational compartments for the WHOLE ODE
# SYSTEM once, not for a particular target, and .g1() applies the chain rule
# to any expression on top of them.
.admBuildThetaSens <- function(ui, unpaired, pred_expr = NULL, order = 1L) {
  ...
  # request promotes the model to its ODE form and builds from that.
  # Promotion failing is not fatal -- NULL here means the caller retries at
  # order 1.
  #
  # DETECTED WITH rxode2::testRxLinCmt(), not by reading predDf$linCmt
  # directly: on rxode2 5.1.4 that column is FALSE for a genuine `cp <-
  # linCmt()` model (the solved form is marked on `ui$.linCmtM` instead), so
  # the gate never fired and EVERY order-2 request on a linCmt model returned
  # NULL, silently dropping adfo back to the FD pass it was written to
  # replace. testRxLinCmt() is EXPORTED, checks both markers, and stays clear
  # of the documented Windows GC/finalizer hazard that made this read predDf
  # originally. A PROMOTED solved-form linCmt is caught instead by the
  # linCmtB text check below.
  if (order >= 2L) {
  ...
  res <- tryCatch({
    ...
    if (length(st) > 0L) {
      ...
      # compartment is generated. .rxSens also assigns the
      # rx__sens_*_BY_*_BY_*__ symbols into `s` as a side effect, which is
      # what makes them resolvable in .g2 below, so this must run BEFORE the
      # chains are built.
      #
      # Requested one eta ROW at a time, each against only the directions at
      # or after it, because the eta x eta half is SYMMETRIC and
      # rxExpandSens2_ does not know it: asked for the full rectangle it
      # emits both d2/(d eta_1 d eta_2) and d2/(d eta_2 d eta_1) as separate
      # variational compartments carrying the same equation -- 18 of 93
      # extra states on a 3-state/4-eta/2-theta model for no information. The
      # eta x theta half has no such symmetry and is requested in full.
      if (order >= 2L) {
    ...
    # Endpoint routing for a MULTI-ENDPOINT model. Its rx_pred_ is a
    # CMT-conditional expression, so the solve needs the endpoint
    # pseudo-compartments declared and mapped, as nlmixr2est's inner model
    # does with `cmt(cp); cmt(ct); dvid(3,4);`. `..stateInfo` is not
    # populated on ui$loadPruneSens, so the lines are built from ui$predDf
    # instead. Single-endpoint models need no routing and get no lines.
    #
    # The BASE states must also be declared up front, as nlmixr2est's inner
    # model does: that pins them to compartments 1..n_base so the endpoint
    # numbering is n_base + i. Declared implicitly (by d/dt alone) the
    # rx__sens_* compartments get interleaved and dvid() resolves wrong.
...
# columns, recompiled with eventSens = "jump" -- WITHOUT that flag a
# parameter entering a dosing modifier (f/lag/rate/dur) has a sensitivity of
# exactly ZERO, silently, since FOCEI computes event/dose sensitivities
# separately and admixr2 reads the inner model's columns directly.
#
# `order`: 1L (default) emits d(pred)/d(dir) only -- what admc/adgh need. 2L
# adds the cross second-order block d2(pred)/(d eta d dir) that adfo needs
# for dJ/dtheta; a failed order-2 build falls back to order 1.
# Would .admLoadSensModel() return NULL for this model BY DESIGN?
#
# It returns NULL for two reasons callers couldn't otherwise tell apart:
# correct, permanent refusals (below), and genuine failures -- a compile
# error inside .admBuildThetaSens(), or an unwritable rxTempDir() where
# .admCacheWrite() fails and the caller's tryCatch(error=) discards a model
# that actually compiled. A user on a locked-down HPC home directory would
# then silently get a coarser struct-theta gradient, converging to
# different estimates and SEs than the same script on a writable machine.
# So the drivers ask this first: by-design NULL stays a message, an
# unexplained NULL becomes a warning.
#
# Deliberately a SEPARATE predicate rather than a reason code returned from
# .admLoadSensModel(): that function's body is digested into the sens cache
# key (see .admPkgKey), so touching it to add plumbing would invalidate
# every cached model for a change that emits nothing. Mirrors the guards in
# .admLoadSensModel(); if one moves, this moves with it.
...
.admLoadSensModel <- function(ui, order = 1L) {
  ...
  # Session cache, ahead of the disk cache and on the same key. Same three
  # reasons as .admLoadModel: skip a readRDS + dyn.load, stop minting a
  # finalizer-bearing object per call, and keep the compiled model reachable.
  # Gated on the disk file so a cache clear still forces a rebuild.
  #
  # The four fields below are re-derived on a hit, NOT trusted from the
  # cached object. The key includes the ini NAMES and fix flags but not
  # their VALUES, so `lam <- 0.5` and `lam <- 0.7` collide; serving a stale
  # pred_tbs left the objective bit-identical and the gradient 1e2-1e4x
  # wrong. Name maps are re-derived from the frame the CACHED build numbered
  # its directions from, not from this call's `ini_df` -- they differ only
  # across an order-2 linCmt promotion, where rebuilding from ini_df would
  # mis-slot each theta against the derivative that differentiates it.
  ...
  # The sensitivity model's rx_pred_ is on the endpoint's MODELLING scale,
  # which for an lnorm endpoint is the LOG scale:
  #
  #   cp ~ add(a)   ->  rx_pred_ = <cp>              (natural)
  #   cp ~ lnorm(a) ->  rx_pred_ = log(<cp>)         (log!)
  #
  # .admSimulate (the NLL path) always reads the natural-scale output column,
  # so an lnorm model had .admGrad differentiating log(f) while .admNLL
  # scored f. EVERY transformed endpoint does this: logit/probit/boxCox/
  # yeoJohnson all emit rx_pred_ = rxTBS(f, ...), and the original fix
  # handled only the log case. The solve paths back-transform with the chain
  # rule; lnorm is exactly yj = 0 with lambda = 0.
  #
  ...
  #
  # A delay() model's sensitivity system is the base ODEs plus one
  # variational compartment per state per direction, all delayed -- stiff
  # enough to trip rxode2's hasDelay AutoSwitch composite into its ros4 leg,
  # whose dense delay history is inaccurate for this system. Not an error:
  # the augmented prediction agrees with the fit's own solve for the first
  # observations and then drifts once delay() starts reading the RECORDED
  # history rather than the pre-history. dop853's 8th-order dense output
  # reproduces the base solve exactly. Mirrors nlmixr2est's ed03b8dfc, which
  # found the same failure in its own augmented-sensitivity solve.
  #
  # Stored on the result -- and folded into the cache key above via
  # .admPkgKey() -- because a parallel worker reads the file directly and
  # cannot re-derive it. NULL for an ordinary model.
