...
# worker from the STALE INSTALLED namespace, so a new formal on `.admRestartWorker`
# (or its three siblings) throws `unused argument` before the patched dev body can
# run -- but `pinfo` is sent by value and carries anything.
#
# Single-sourced because the set GROWS. Each of `sigdig`, `sim_cache_file` and
# `resid_nodes` was added to four drivers by hand, and a field set in three of them
# is not an error anywhere -- it is a silent divergence in whichever estimator was
# missed. adirmc adds `grad_h` of its own after this returns.
.admDriverPinfo <- function(.ui, .ctl) {
  ...
  # Nodes for the COVARIATE dimension of the adgh product grid. Read in exactly one
  # place (.admCovGrid via .adghGrid) and, until this line existed, set nowhere -- so
  # the `%||% 7L` fallback always fired and the covariate integration was fixed at 7
  # nodes. That is what made the accuracy sweep plateau: raising n_nodes refines only
  # the random-effect dimensions.
  ...
  # reduced wherever a reduction verifies), "off" (that grid, unreduced) or "sparse"
  # (a Smolyak grid over the same margins). Only adghControl() exposes it -- the
  # reductions are quadrature constructions and the other estimators reach their etas
  # through .admMakeZ, so "off" is the only correct value for them.
...
#
# `multi_out` is MODEL-level (`length(.admOutputVars(ui)) > 1`), not study-level, so
# a multi-endpoint model always tags observations by `cmt` -- the solve cannot
# disambiguate endpoints otherwise. `any_joint` is study-level and gates the paths
# that have no joint implementation.
#
# `tag_cmt` defaults to `multi_out`, which is what three drivers pass. adirmc refuses
# both multi-output and joint studies BETWEEN the flatten and the ev_full build, so
# it calls this with `ev_full = FALSE` and builds ev_full itself.
...
#
# A model that READS a covariate needs that covariate present here, or rxode2 stops
# with "the following parameter(s) are required for solving" AFTER the whole
# optimisation has run, so the fit is lost at the last step. The frame is built by
# the driver, not taken from the caller, so adding the column to the data passed to
# nlmixr2() does not help.
#
# The VALUE is immaterial: this row carries the non-NA placeholder DV and never
# enters the reported objective, which every estimator overwrites with its own
# aggregate -2LL. It must merely be finite and not something the model divides by.
...
.admFinaliseFit <- function(.ret, .ui, .ctl, est, objective, ov, studies,
  ...
  # The stratum resolution the studies were generated at, if any. Recorded HERE
  # because all four drivers pass through this function -- a field set in three of
  # four is a silent divergence, not an error. The objective is J-dependent, so
  # anova() refuses to compare fits that disagree on it. Stamped on .fit below, NOT
  # on .ret: `.ret$env` does not exist, so assigning into it creates a plain LIST
  # named `env` and the field is unreachable, which is why anova()'s refusal was dead
  # code. The whole set is recorded, so a mixed-resolution fit is comparable only
  # with another built the same way.
