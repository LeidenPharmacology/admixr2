...
.admAggData <- function(extra, ui, n_sim = NULL, seed = 1L, warn = TRUE) {
  ...
  .sim_study <- function(s) {
    ...
    # A SHIFT study's whole covariate effect rides in the shifted eta column,
    # correct for the estimator but wrong here where etas are ordinary Omega
    # draws -- left untouched, every panel described a model with no covariate
    # effect. Force the general per-row path (always valid, just slower; cost
    # doesn't matter for one diagnostic draw): without it the solve succeeds at
    # the covariate MEAN while the observed V still carries the spread,
    # measured 29-35% low predicted covariances (61% off-diagonal).
