...
.admLRT <- function(full, reduced) {
  ...
  # A J-DEPENDENT OBJECTIVE CANNOT BE COMPARED ACROSS J. On the default banding
  # rule the objective is in fact stable (0.03 units across J = 5 to 100), so
  # this rarely binds -- but an opaque `joint` sampler falls back to
  # equiprobable bins, where the objective was still moving 51 units at J = 100
  # and the estimate itself wandered. Refusing is cheap and the alternative is a
  ...
  if (!is.null(.j1) && !is.null(.j2) && !identical(.j1, .j2))
