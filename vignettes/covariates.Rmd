...
* `mild` **does** use it, so `CRCL` is **conditional** for that source and the
  paper can be read at values of it.
...
All three fitted a sex effect, so all three band on sex too. Banding is not
...
knitr::kable(data.frame(
  form      = c("power (correct)", "linear"),
  `max abs z` = c(max(abs(m_pow$z)), max(abs(m_lin$z))),
  `obs/pred SD, worst` = c(max(abs(m_pow$obs_sd / m_pow$pred_sd - 1)),
                           max(abs(m_lin$obs_sd / m_lin$pred_sd - 1))),
  check.names = FALSE),
  digits = 3, row.names = FALSE,
  caption = "The wrong form roughly doubles the worst residual.")
```
...
kg, so their bars lie on top of one another and the facet cannot speak to weight
