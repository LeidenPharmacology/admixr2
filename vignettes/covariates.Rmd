...
narrow bands of renal function, so within those trials the effect is invisible,
...
enrolled narrow renal bands and fitted no renal term; Khan enrolled across
...
cohorts <- list(normal   = draw_cohort(260L, 78, .198, 95, .05),
                ...
                # band -- which is why Khan's analysts could estimate a renal
...
* `normal` and `moderate` estimated no renal effect, so `CRCL` is **marginal**
  ...
  over that. There is no contrast in either paper to band on, and splitting
...
fitted a sex effect, so all three band on sex. Banding is not
...
sources cleanly. `SEX` is banded, so each source contributes a pair and the grey
line joining them is the within-source contrast banding bought. `WT` is the
